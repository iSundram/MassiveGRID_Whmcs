...
<head>
   ...
   {foreach from=$customfields item=customfield}
    {if $customfield.fieldname eq "Brand" && $customfield.value|strstr:"INC"}
        <script>
            document.addEventListener("DOMContentLoaded", function() {
                // Find all strong tags and hide the one containing 'Payment Method' text
                const strongElements = document.querySelectorAll("strong");
                strongElements.forEach(function(strong) {
                    if (strong.innerText.includes("{lang key='paymentmethod'}")) {
                        strong.style.display = "none";
                    }
                });

                // Locate the payment methods section by its role and hide it
                const paymentMethodsField = document.querySelector("[data-role='paymethod-info']");
                if (paymentMethodsField) {
                    paymentMethodsField.style.display = "none";
                }
            });
        </script>
    {/if}
            ...
            <div class="row justify-content-sm-between">
                <div class="col-12 col-sm-6 order-sm-last text-sm-right invoice-col right">
                   ...
                   <address class="small-text">
                    {foreach from=$customfields item=customfield}
                        {if $customfield.fieldname eq "Brand" && $customfield.value|strstr:"INC"}
                            {$payto = $LANG.MassiveGRID.INC.PayTo.text}
                        {/if}
                    {/foreach}
                    {$payto}
