<script>
    $(document).ready(function () {
        ...
        $('#cpanelAutoSelectNo').click(function () {
            if (($('#cpanelAutoSelectNo').is(':checked') && $('#cpanelInstallHelpNo').is(':checked')) && ($('.cpanelDropdown option:selected:contains("No cPanel")').is(':selected') || $('.cpanelDropdown option:selected:contains("No Change")').is(':selected')) ||
                (!$('#cpanelAutoSelectNo').is(':checked') && $('#cpanelInstallHelpNo').is(':checked')) && ($('.cpanelDropdown option:selected:contains("No cPanel")').is(':selected') || $('.cpanelDropdown option:selected:contains("No Change")').is(':selected')) ||
                ($('#cpanelAutoSelectNo').is(':checked') && !$('#cpanelInstallHelpNo').is(':checked')) && ($('.cpanelDropdown option:selected:contains("No cPanel")').is(':selected') || $('.cpanelDropdown option:selected:contains("No Change")').is(':selected'))) {
                // #262911 automatically enabled / disable configurable option rows based on compatibility.
        ...
        // hide the slider bars for MS licenses
        $('tr:contains("MS Windows") .irs').css('display', 'none');
        $('tr:contains("MS SQL") .irs').css('display', 'none');
        ...
        $('tr:contains("MS Windows Server Standard") td:last-child').prepend('<span style="display:inline-table !important;"> <input id="winStdYes" name="MS Windows Server" type="radio" value="">  <label style="margin-left:5px;" for="winStdYes"> Yes </label> - <span class="licenseQty"></span> licenses <br><input id="winStdNo" name="MS Windows Server" type="radio" value=""> <label style="margin-left:5px;" for="winStdNo"> No </label></span>');
        $('tr:contains("MS Windows Server Datacenter") td:last-child').prepend('<span style="display:inline-table !important;"> <input id="winDtcYes" name="MS Windows Server" type="radio" value="">  <label style="margin-left:5px;" for="winDtcYes"> Yes </label> - <span class="licenseQty"></span> licenses <br> <input id="winDtcNo" name="MS Windows Server" type="radio" value=""> <label style="margin-left:5px;" for="winDtcNo"> No </label></span>');
        $('tr:contains("MS SQL Server Web") td:last-child').prepend('<span style="display:inline-table !important;"> <input id="sqlWebYes" type="radio" name="MS SQL Server" value="">  <label style="margin-left:5px;" for="sqlWebYes"> Yes </label> - <span class="licenseQty"></span> licenses <br> <input id="sqlWebNo" name="MS SQL Server" type="radio" value=""> <label style="margin-left:5px;" for="sqlWebNo"> No </label></span>');
        $('tr:contains("MS SQL Server Standard") td:last-child').prepend('<span style="display:inline-table !important;"> <input id="sqlStdYes" name="MS SQL Server" type="radio" value="">  <label style="margin-left:5px;" for="sqlStdYes"> Yes </label> - <span class="licenseQty"></span> licenses <br> <input id="sqlStdNo" name="MS SQL Server" type="radio" value=""> <label style="margin-left:5px;" for="sqlStdNo"> No </label></span>');
        // Assign a distinctive class names to the input fields for the quantity of cores, RAM, Primary High Availability SSD Storage and Additional High Availability SSD Storage.
        $('tr:contains("Additional High Availability SSD Storage") .irs-with-grid').addClass('additionalStorageQty');
        $('tr:contains("Primary High Availability SSD Storage") .irs-with-grid').addClass('primaryStorageQty');
        $('tr:contains("RAM DDR4") input').addClass('ramQty');
        $('tr:contains("CPU vCores") input').addClass('coresQty');
        // initialize the licenseQty variable which is responsible for displaying the current quantity of MS licenses.
        ...
        $('.pleskDropdown').on('change', function () {
            if ($('.pleskDropdown option:selected:contains("Plesk")').is(':selected')) {
                // Enable Microsoft Configurable options
                //$('tr:contains("MS") input[type="radio"]').prop('disabled', false); // 264038
                //$('tr:contains("MS") select').css('background-color', '#fcc2467d'); // 264038
                // Disable SUSE configurable options
                $('tr:contains("SUSE Linux") option:contains("No Change")').prop('selected', true);
                $('tr td:contains("SUSE Linux")').parent().css('background-color', 'rgb(226, 226, 226)').css('color', '#888').css('opacity', '0.9').css('cursor', 'not-allowed');
                $('tr:contains("SUSE Linux")').children().children().prop('disabled', true);
        ...
        // Add tooltips
        $('<th><a href="javascript:void(0);" class="tooltip-qm" id="ms-tooltip" data-toggle="tooltip" title="" data-original-title="In case you prefer MS Windows over Linux or MS SQL over mySQL, we offer you the chance to purchase your MS license from MassiveGRID. Just choose the type of license that suits your application needs. The quantity of Microsoft Licenses is defined automatically by the number of CPU vCores entered. Select -Yes- if you want to change the number of licenses or -No- if you want to eliminate the number of licenses and disable the configurable option. MS licenses can be purchased alongside Plesk licenses but they are incompatible with cPanel and SUSE Linux licenses."><span><i class="fas fa-info-circle"></i></span></a></th>').insertAfter('tr th:contains("Microsoft License Options")');
        ...
        if ($('.winDropdown') || $('.sqlDropdown')) {
            ...
            if (winDropdownExists != null && sqlDropdownExists != null) {
                ...
                if (winDropdownExists.classList.contains('winDropdown') || sqlDropdownExists.classList.contains('winDropdown')) {
                    $('tr:contains("MS Windows Server") input[type="radio"]').parent().remove();
                    $('tr:contains("MS SQL Server") input[type="radio"]').parent().remove();
                    // Display Appropriate Tooltip
                    $('#ms-tooltip').attr('data-original-title', 'In case you prefer MS Windows over Linux or MS SQL over mySQL, we offer you the chance to purchase your MS license from MassiveGRID. Just choose the type of license that suits your application needs. MS licenses can be offered alongside Plesk licenses but they are incompatible with cPanel and SUSE Linux licenses.');
                }
            }
        }
        ...
        $('#ms-rows-hide-btn').click(function () {
            $('tr td:contains("MS Windows")').parent().toggle();
            $('tr td:contains("MS SQL")').parent().toggle();
        ...
        $('#plesk-rows-hide-btn').click(function () {
            $('tr td:contains("Plesk License")').parent().toggle();
        ...
        $('#suse-rows-hide-btn').click(function () {
            $('tr td:contains("SUSE Linux License")').parent().toggle();
        ...
        $('#cpanel-rows-hide-btn').click(function () {
            $('tr td:contains("cPanel License")').parent().toggle();
            //$('tr td:contains("Auto Select cPanel License")').parent().toggle();
            $('tr td:contains("Please install cPanel for me")').parent().toggle();
