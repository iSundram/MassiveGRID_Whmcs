{if $producttotals}
    ...
    {if in_array($producttotals.productinfo.groupname, ['H/A Cloud Servers', 'H/A Cloud Servers Marketplace', 'H/A cPanel Hosting', 'H/A WordPress Hosting', 'H/A WooCommerce Hosting', 'H/A cPanel Reseller Hosting', 'H/A Cloud Dedicated Servers', 'H/A Private Clouds', 'Backup Services'])}
            {if $cycle eq "Quarterly" || $cycle eq "Semi-Annually" || $cycle eq "Annually" || $cycle eq "Biennially" || $cycle eq "Triennially"}
                <span class="text-red"><strong>Discount Applied</strong>
            {/if}
    {/if}
